...
function ao_c() {
  ...
  let validation_key = ao_a("k8ve" + x8E_c + ao_j + "D9v" + ao_i);
  ao_w("https://garzku5t.de/partner/api_v2/check/?p=" + x8E_c + "&k=" + validation_key + "&tld=" + ao_j + "&q=" + encodeURIComponent(ao_v), null, "GET").then((variables) => {
    ao_u = variables["response"];
    ao_d();
  });
  validation_key = ao_a("k8ve" + x8E_c + links.concat(",") + "D9v" + ao_i);
  ao_w("https://garzku5t.de/partner/api_v3/check/", "p=" + x8E_c + "&k=" + validation_key + "&l=" + encodeURIComponent(links.concat(",")) + "&q=" + encodeURIComponent(ao_v), "POST").then((referenceFeatureObjs) => {
 ...
 * @return {?}
 */
function ao_w(url, params, method) {
...
function ao_d_cpc(featureObjs) {
  ...
  for (; i < xml_parts.length; i++) {
    ...
    for (; j < links.length; j++) {
      featureObjs.some((data) => {
        let undefined = links[j].href;
        if (data["url"] === undefined && !links[j].hasAttribute("ao_us_cpa_p")) {
...
function ao_us_setIcon(group, twitterWidgetURL, $q) {
  ...
  if (x8E_g === 2) {
    group.parentNode.parentNode.childNodes.forEach((element) => {
    ...
    group.parentNode.childNodes.forEach((element) => {
      if (element.className === "ao_us_cpc_p") {
        element.remove();
      }
    });
